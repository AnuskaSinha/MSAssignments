...
import morganStanley_Day3.User;
...
//		employees.stream().filter(e->e.getDateOfBirth().getYear() == 2023).map(Employee::getFirstName).forEach(printFirstName);
//		System.out.println("----------------------------------------");
//		
...
//		String dept = sc.next();
//		DoubleSummaryStatistics dSS = employees.stream().filter(e->empWithReqDeptUsingBP.test(e, dept)).mapToDouble(Employee::getSalary).summaryStatistics();
...
//		Map<String, DoubleSummaryStatistics> dSSMapValues = employees.stream().collect(Collectors.groupingBy(Employee::getDept, Collectors.summarizingDouble(Employee::getSalary)));
//		dSSMapValues.forEach((department, stats)->{
...
//		System.out.println("----------------------------------------");
//		
//		// Q4. Sorted list of employees by firstName in all departments except HR department
//		System.out.println("Sorted list of employees by firstName in all departments except HR department");
//		employees.stream().filter(e->!e.getDept().equals("HR")).sorted(Comparator.comparing(Employee::getFirstName)).forEach(printEmp);
//		System.out.println("----------------------------------------");
//		
...
//		employees.stream().filter(e->empWithReqDeptUsingBP.test(e, dept1)).forEach(e->e.setSalary(e.getSalary()*1.1));
...
//		System.out.println("----------------------------------------");
		...
		String empList = employees.stream().sorted(Comparator.comparing(Employee::getDateOfBirth)).map(Employee::getFirstName).collect(Collectors.joining(", "));
